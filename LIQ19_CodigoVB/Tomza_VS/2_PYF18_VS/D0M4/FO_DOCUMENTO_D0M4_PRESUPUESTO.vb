...
Public Class FO_DOCUMENTO_D0M4_PRESUPUESTO
...
#Region "SUBS_PM"
    ...
    Private Sub PM_LISTADO_CLICK(ByRef PP_FORMA As Object, ByRef PP_LISTADO As DataGridView)

        Dim VP_ID As String
        Try

            If Codigo_LI.FG_LI_ROW_SELECTED(PP_LISTADO) Then
                If PP_FORMA.VM_IN_MODO_OPERACION = Modo_Operacion.Listado Then 'este if es por si esta viendo la informacion de la ficha y se quiere editar
                    Dim VP_ROW As Integer = Codigo_LI.FG_LI_ROW_CURRENT(PP_LISTADO)

                    VP_ID = PP_FORMA.FM_ID_SELECCIONADO(PP_LISTADO, VP_ROW)

                    FO_ESTIMACION.PM_FO_SHOW("DOCUMENTO_PRESUPUESTO", VM_ID_BASE_DE_DATOS, VP_ID)


                End If
            End If
        Catch ex As Exception
            Codigo_Message.PG_MENSAJE_ERROR_VS("PM_LISTADO_CLICK")
        End Try
...
#Region "EVENTOS"
    ...
    Private Sub LI_LISTADO_DoubleClick(sender As Object, e As EventArgs) Handles LI_LISTADO.DoubleClick
        ...
        PM_LISTADO_CLICK(Me, LI_LISTADO)
    End Sub
