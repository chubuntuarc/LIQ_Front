Public Class FO_CONTROL_ANUAL_UNIDAD_OPERATIVA
...
#Region "FUNCIONES_FM"
    ...
    Private Function FM_VALIDAR_AÑO_Y_UNIDAD_OPERATIVA() As String
        ...
        End If
        If Codigo_CB.FG_CB_ITEM_VALUE(CB_LI_UNIDAD_OPERATIVA) = -1 Then
            If PP_MENSAJE_VALIDACION = "" Then
                PP_MENSAJE_VALIDACION = "Seleccione la Unidad Operativa a la que se agregará el año."
            Else
                PP_MENSAJE_VALIDACION += vbCrLf + "Y seleccione la Unidad Operativa a la que se agregará el año."
            End If
...
#Region "SUBS_PM"
    ...
    Public Sub PM_FI_CARGAR(ByVal PP_ROW As DataRow)
        ...
        Codigo_CTRL.PG_CO_DATA_LOAD("K_UNIDAD_OPERATIVA", CB_UNIDAD_OPERATIVA, PP_ROW)
    ...
    Private Sub PM_LI_FORMAT(ByRef PP_LI_LISTADO As DataGridView)
        ...
        Codigo_LI.PG_LI_COLUMN_ADD(PP_LI_LISTADO, "S_ZONA_UO", "Zona", GetType(String), 50, 2, 0, 1, 1, 0, Color.DarkOrange, Nothing, 1)
        Codigo_LI.PG_LI_COLUMN_ADD(PP_LI_LISTADO, "K_UNIDAD_OPERATIVA", "#UO", GetType(Integer), 40, 2, 0, 0, 0, 0)
        Codigo_LI.PG_LI_COLUMN_ADD(PP_LI_LISTADO, "D_UNIDAD_OPERATIVA", "Unidad Operativa", GetType(String), 200, 1, 0, 1, 1, 0, Nothing, Nothing, 1)
    ...
    Public Sub PM_CARGAR_FICHA(ByRef PP_LISTADO As DataGridView)
        ...
        Try
            If PP_LISTADO.Rows.Count > 0 Then
                ...
                Dim VP_UNIDAD_OPERATIVA As Integer = Codigo_LI.FG_LI_CELL_READ(PP_LISTADO, VP_ROW, "K_UNIDAD_OPERATIVA")
                Dim VP_IDS As String = VP_ID.ToString + ", " + VP_MES.ToString + ", " + VP_UNIDAD_OPERATIVA.ToString
                PM_FI_LOAD(VP_IDS)
    ...
    Public Function FG_LI_COLUMN_CURRENT(ByVal PP_LISTADO As DataGridView) As Integer
        ...
        If PP_LISTADO.CurrentCell IsNot Nothing Then
            ...
            If VP_COLUMN = 4 Then VP_MES = 1
            If VP_COLUMN = 5 Then VP_MES = 2
            If VP_COLUMN = 6 Then VP_MES = 3
            If VP_COLUMN = 7 Then VP_MES = 4
            If VP_COLUMN = 8 Then VP_MES = 5
            If VP_COLUMN = 9 Then VP_MES = 6
            If VP_COLUMN = 10 Then VP_MES = 7
            If VP_COLUMN = 11 Then VP_MES = 8
            If VP_COLUMN = 12 Then VP_MES = 9
            If VP_COLUMN = 13 Then VP_MES = 10
            If VP_COLUMN = 14 Then VP_MES = 11
            If VP_COLUMN = 15 Then VP_MES = 12
        End If
        Return VP_MES
    End Function
    ...
    Public Sub PM_BT_BUSCAR_CLICK(ByRef PP_FORMA As Object, ByVal PP_LISTADO As DataGridView)
        PM_LI_FORMAT(LI_LISTADO)
        Codigo_ABC.PG_BT_BUSCAR_CLICK(VM_ID_BASE_DE_DATOS, PP_FORMA, PP_LISTADO, "[PG_LI_MATRIZ_CONTROL_X_MES_X_UNIDAD_OPERATIVA]")
    End Sub
    ...
    Public Sub PM_BT_AGREGAR_AÑO_CLICK(ByRef PP_FORMA As Object)
        Dim VM_MSJ As String = FM_VALIDAR_AÑO_Y_UNIDAD_OPERATIVA()
        If VM_MSJ = "" Then
            Dim PP_PARAMETROS As String = FM_GENERAR_AÑO()
            Codigo_CON.FG_SQL_EJECUTAR_ACCION(PP_FORMA, VM_ID_BASE_DE_DATOS, "[PG_IN_MATRIZ_CONTROL_X_MES_X_UNIDAD_OPERATIVA]", PP_PARAMETROS)
        Else
    ...
    Public Sub PM_BT_GENERAR_BLOQUE_CONTROL_CLICK(ByRef PP_FORMA As Object)
        ...
        If VM_MSJ = "" Then
            ...
            Codigo_CON.FG_SQL_EJECUTAR_ACCION(Me, VM_ID_BASE_DE_DATOS, "[]", PP_PARAMETROS)
        Else
            Codigo_Message.PG_MENSAJE_GUARDAR_VALIDAR(VM_MSJ)
        End If
        PM_BT_BUSCAR_CLICK(PP_FORMA, LI_LISTADO)
