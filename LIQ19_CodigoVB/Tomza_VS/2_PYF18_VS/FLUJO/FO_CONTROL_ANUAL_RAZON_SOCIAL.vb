Public Class FO_CONTROL_ANUAL_RAZON_SOCIAL
...
#Region "FUNCIONES_FM"
    ...
    Public Function FM_SQL_LISTADO() As String
        ...
        Codigo_CTRL.PG_CO_PARAMETRO(VP_PARAMETROS, CB_LI_ESTATUS_RAZON_SOCIAL, False)
    ...
    Public Function FM_FI_SQL(ByRef PP_NUEVO As Boolean)
        ...
        Codigo_CTRL.PG_CO_PARAMETRO(VP_PARAMETROS, CH_L_05__PFD_POLIZA_EDIT, False)
        Codigo_CTRL.PG_CO_PARAMETRO(VP_PARAMETROS, CH_L_06__PFD_INGRESOS_ADD, False)
    ...
    Private Function FM_VALIDAR_AÑO_Y_RAZON_SOCIAL() As String
        Dim PP_MENSAJE_VALIDACION As String = ""
        If Codigo_CB.FG_CB_ITEM_VALUE(CB_LI_AÑO) = -1 Then
            PP_MENSAJE_VALIDACION = "Seleccione un año válido."
        End If
        If Codigo_CB.FG_CB_ITEM_VALUE(CB_LI_RAZON_SOCIAL) = -1 Then
            If PP_MENSAJE_VALIDACION = "" Then
                PP_MENSAJE_VALIDACION = "Seleccione la Razón Social a la que se agregará el año."
            Else
                PP_MENSAJE_VALIDACION += vbCrLf + "Y seleccione la Razón Social a la que se agregará el año."
            End If
...
#Region "SUB_PM_INIT"
    ...
    Public Sub PM_CB_INIT()
        ...
        Codigo_CB.PG_CB_LOAD_X_ORDEN(VM_ID_BASE_DE_DATOS, Me, CB_LI_ESTATUS_RAZON_SOCIAL, "ESTATUS_RAZON_SOCIAL")
        Codigo_CB.PG_CB_RAZON_SOCIAL_Load(VM_ID_BASE_DE_DATOS, Me, CB_LI_RAZON_SOCIAL, 1, CB_LI_ESTATUS_RAZON_SOCIAL, New ComboBox, VG_USUARIO_ACCION)
...
#Region "SUBS_PM"
    ...
    Public Sub PM_FI_CARGAR(ByVal PP_ROW As DataRow)
        ...
        Codigo_CTRL.PG_CO_DATA_LOAD("K_RAZON_SOCIAL", CB_RAZON_SOCIAL, PP_ROW)
        ...
        Codigo_CTRL.PG_CO_DATA_LOAD("L_05_PFD_POLIZA_EDIT", CH_L_05__PFD_POLIZA_EDIT, PP_ROW)
        Codigo_CTRL.PG_CO_DATA_LOAD("L_06_PFD_INGRESOS_ADD", CH_L_06__PFD_INGRESOS_ADD, PP_ROW)
    ...
    Private Sub PM_LI_FORMAT(ByRef PP_LI_LISTADO As DataGridView)
        ...
        Codigo_LI.PG_LI_COLUMN_ADD(PP_LI_LISTADO, "K_RAZON_SOCIAL", "#RS", GetType(Integer), 40, 2, 0, 0, 0, 0)
        Codigo_LI.PG_LI_COLUMN_ADD(PP_LI_LISTADO, "D_RAZON_SOCIAL", "Razón Social", GetType(String), 250, 1, 0, 1, 1, 0, Nothing, Nothing, 1)
        Codigo_LI.PG_LI_COLUMN_ADD(PP_LI_LISTADO, "S_REGION", "Región", GetType(String), 60, 2, 0, 1, 1, 0, Color.DarkOrange, Nothing, 1)
        Codigo_LI.PG_LI_COLUMN_ADD(PP_LI_LISTADO, "S_ESTATUS_RAZON_SOCIAL", "Estatus", GetType(String), 60, 2, 0, 1, 1, 0, Color.DarkSlateGray, Color.Gainsboro, 1)
    ...
    Public Sub PM_CARGAR_FICHA(ByRef PP_LISTADO As DataGridView)
        ...
        Try
            If PP_LISTADO.Rows.Count > 0 Then
                ...
                Dim VP_RAZON_SOCIAL As Integer = Codigo_LI.FG_LI_CELL_READ(PP_LISTADO, VP_ROW, "K_RAZON_SOCIAL")
                Dim VP_IDS As String = VP_ID.ToString + ", " + VP_MES.ToString + ", " + VP_RAZON_SOCIAL.ToString
                PM_FI_LOAD(VP_IDS)
    ...
    Public Function FG_LI_COLUMN_CURRENT(ByVal PP_LISTADO As DataGridView) As Integer
        ...
        If PP_LISTADO.CurrentCell IsNot Nothing Then
            ...
            If VP_COLUMN = 5 Then VP_MES = 1
            If VP_COLUMN = 6 Then VP_MES = 2
            If VP_COLUMN = 7 Then VP_MES = 3
            If VP_COLUMN = 8 Then VP_MES = 4
            If VP_COLUMN = 9 Then VP_MES = 5
            If VP_COLUMN = 10 Then VP_MES = 6
            If VP_COLUMN = 11 Then VP_MES = 7
            If VP_COLUMN = 12 Then VP_MES = 8
            If VP_COLUMN = 13 Then VP_MES = 9
            If VP_COLUMN = 14 Then VP_MES = 10
            If VP_COLUMN = 15 Then VP_MES = 11
            If VP_COLUMN = 16 Then VP_MES = 12
        End If
        Return VP_MES
    End Function
    ...
    Public Sub PM_BT_BUSCAR_CLICK(ByRef PP_FORMA As Object, ByVal PP_LISTADO As DataGridView)
        PM_LI_FORMAT(LI_LISTADO)
        Codigo_ABC.PG_BT_BUSCAR_CLICK(VM_ID_BASE_DE_DATOS, PP_FORMA, PP_LISTADO, "[PG_LI_MATRIZ_CONTROL_X_MES_X_RAZON_SOCIAL]")
    End Sub
    ...
    Public Sub PM_BT_AGREGAR_AÑO_CLICK(ByRef PP_FORMA As Object)
        Dim VM_MSJ As String = FM_VALIDAR_AÑO_Y_RAZON_SOCIAL()
        If VM_MSJ = "" Then
            Dim PP_PARAMETROS As String = FM_GENERAR_AÑO()
            Codigo_CON.FG_SQL_EJECUTAR_ACCION(PP_FORMA, VM_ID_BASE_DE_DATOS, "[PG_IN_MATRIZ_CONTROL_X_MES_X_RAZON_SOCIAL]", PP_PARAMETROS)
        Else
    ...
    Public Sub PM_BT_GENERAR_BLOQUE_CONTROL_CLICK(ByRef PP_FORMA As Object)
        Dim VM_MSJ As String = FM_VALIDAR_AÑO()
        If VM_MSJ = "" Then
            Dim PP_PARAMETROS As String = FM_GENERAR_BLOQUE_CONTROL()
            Codigo_CON.FG_SQL_EJECUTAR_ACCION(PP_FORMA, VM_ID_BASE_DE_DATOS, "[]", PP_PARAMETROS)
        Else
            Codigo_Message.PG_MENSAJE_GUARDAR_VALIDAR(VM_MSJ)
        End If
        PM_BT_BUSCAR_CLICK(PP_FORMA, LI_LISTADO)
...
#Region "EVENTOS"
    ...
    Private Sub BT_GENERAR_BLOQUE_CONTROL_Click(sender As Object, e As EventArgs) Handles BT_GENERAR_BLOQUE_CONTROL.Click
        PM_BT_GENERAR_BLOQUE_CONTROL_CLICK(Me)
    End Sub

#End Region

#Region "EVENTOS CHANGE VALUE DE LOS COMBOS DE RAZÓN SOCIAL"

    Private Sub CB_LI_ESTATUS_RAZON_SOCIAL_SelectedValueChanged(sender As Object, e As EventArgs) Handles CB_LI_ESTATUS_RAZON_SOCIAL.SelectedValueChanged
        Codigo_CB.PG_CB_RAZON_SOCIAL_Load(VM_ID_BASE_DE_DATOS, Me, CB_LI_RAZON_SOCIAL, 1, CB_LI_ESTATUS_RAZON_SOCIAL, New ComboBox, VG_USUARIO_ACCION)
    End Sub
